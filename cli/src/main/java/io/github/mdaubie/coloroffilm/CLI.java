...
/**
 * Launch a color-of-film image build with the provided arguments
 * Run with -h or --help argument to display the help
 */
@Command(name = "color-of-film", description = {"Compute a color representation for a single video file"}, mixinStandardHelpOptions = true, sortOptions = false)
public class CLI implements Callable<Integer> {
    @Option(names = {"-fp", "--file_path"}, required = true,
            description = {"Path of the video file to scan"})
    private String filePath;

    @Option(names = {"-r", "--resolution"}, required = true,
